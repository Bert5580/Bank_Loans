...
-- Ensure `debit` column exists in the `players` table for tracking loan debt
ALTER TABLE players 
ADD COLUMN IF NOT EXISTS debit DECIMAL(10,2) DEFAULT 0.00;

-- Ensure `credit` column exists in the `players` table for the credit system
ALTER TABLE players 
...
-- Create the `player_loans` table if it doesn't exist
...
-- Ensure `players` table exists and contains the necessary fields
